...
The plotting functions in RUBer do not in any way extend the
...
The default settings of the RUBer plot functions are optimized for the
corporate design font, RUB Flama, and [Windows Enhanced Metafile
(EMF)](https://en.wikipedia.org/wiki/Windows_Metafile) as
output format. Unfortunately, RUB Flama is not publicly available and cannot be made accessible to Github server rendering this document and creating the example figures. In addition, the images in this document use a raster format, PNG, created by the `{ragg}` device, rather than the EMF vector format created by `{devEMF}`. Practically, all of this means that the default settings of the RUB plot functions do not work well for HTML output, including this document. You are not seeing the intended font, nor do the text sizes match what one would get in Microsoft Word.
...
data, `caption`. By default, the caption has the prefix "*Quelle:*",
which we change from German to English using the parameter
...
`caption` and `caption_prefix`, and a lower threshhold for the
`filter_cutoff`, leading to more labels being displayed.
